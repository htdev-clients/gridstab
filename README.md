...
- **Book purchase system** — End-to-end paid download flow: Stripe Checkout → webhook → per-buyer watermarked PDF → Resend email delivery. Implemented as Cloudflare Pages Functions (`functions/api/`) with a standalone Cloudflare Worker (`workers/watermark/`) as an async queue consumer that watermarks all 94 pages via `pdf-lib`. Uses Cloudflare Queue for async processing, R2 for PDF storage, and KV for delivery idempotency.
