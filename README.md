...
- **Book purchase system** *(infrastructure built, not yet live)* — End-to-end paid download flow: Stripe Checkout → webhook → per-buyer watermarked PDF → Resend email with a permanent download link. Implemented as Cloudflare Pages Functions (`functions/api/`) with a standalone Cloudflare Worker (`workers/watermark/`) as an async queue consumer that watermarks all 94 pages via `pdf-lib`, stores the result in R2, and writes a UUID token to KV. The buy button is currently disabled with a "Coming soon" label pending Stripe/Resend/R2 provisioning.
